...
import { Briefcase, User, GraduationCap, Plus, List, Settings, UserPlus, Car } from 'lucide-react';
...
const KurumSahibiPage: React.FC<KurumSahibiPageProps> = ({ onLogout }) => {
  ...
  const handleAssignAday = () => {
    if (selectedEgitmen && selectedAday) {
      const egitmenRef = ref(database, `egitmenler/${selectedEgitmen}/adaylar`);
      const adayRef = ref(database, `adaylar/${selectedAday}/egitmen`);

      update(egitmenRef, { [selectedAday]: true });
